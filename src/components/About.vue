<template>
  <div>
    <div class="container">
      <div>
        <div>
          <h2 class="white-text">Malayalam morphological analyser</h2>
          <p>mlmorph is Malayalam Morphology Analyzer and Generator.</p>
        </div>
        <div>
          <h5>Source code</h5>
          <ul>
            <li>
              <a href="https://gitlab.com/smc/mlmorph">Morphology analyser</a>
            </li>
            <li>
              <a href="https://gitlab.com/smc/mlmorph-spellchecker"
                >Malayalam spellchecker</a
              >
            </li>
            <li>
              <a href="https://gitlab.com/smc/mlmorph-web"
                >This web interface</a
              >
            </li>
            <li>
              <a href="https://gitlab.com/smc/mlmorph-libreoffice-spellchecker"
                >Libreoffice spellchecker</a
              >
            </li>
          </ul>
        </div>
      </div>
    </div>
    <div class="footer-copyright">
      <div class="container">
        ©
        <a href="https://thottingal.in">Santhosh Thottingal</a>
      </div>
    </div>
  </div>
</template>
